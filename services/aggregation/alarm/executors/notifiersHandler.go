...
func (handler *notifiersHandler) NotifyWithRetry(caller string, messages ...common.OutputMessage) error {
	...
	notifiersThatErrored := make(map[string]Notifier)
	handler.handleFirstSend(notifiersThatErrored, messages)
	return handler.handleRetries(notifiersThatErrored, messages)
}

func (handler *notifiersHandler) handleFirstSend(notifiersThatErrored map[string]Notifier, messages []common.OutputMessage) {
	if len(messages) == 0 {
		return
	}
...
func (handler *notifiersHandler) handleRetries(notifiersThatErrored map[string]Notifier, messages []common.OutputMessage) error {
...
func (handler *notifiersHandler) notifyInRetryMode(retryNumber uint32, notifiersThatErrored map[string]Notifier, messages []common.OutputMessage) {
...
func uniqueNotifierName(notifier Notifier) string {
