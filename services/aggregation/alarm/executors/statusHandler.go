package executors

import (
	"context"
	"fmt"
	"sync"
	"sync/atomic"
	"time"

	"github.com/iulianpascalau/api-monitoring/services/aggregation/common"
	"github.com/multiversx/mx-chain-core-go/core/check"
)

const statusHandlerName = "statusHandler"

type statusHandler struct {
	mut              sync.Mutex
	numErrors        uint32
	problematicKeys  map[string]struct{}
	notifiersHandler OutputNotifiersHandler
}

// NewStatusHandler creates a new instance of type statusHandler
func NewStatusHandler(notifiersHandler OutputNotifiersHandler) (*statusHandler, error) {
	if check.IfNil(notifiersHandler) {
		return nil, errNilOutputNotifiersHandler
	}

	handler := &statusHandler{
		notifiersHandler: notifiersHandler,
		problematicKeys:  make(map[string]struct{}),
	}

	return handler, nil
}

// NotifyAppStart will send a message on all notifiers stating that the application was started
func (handler *statusHandler) NotifyAppStart() {
	msg := common.OutputMessage{
		Type:         common.InfoMessageOutputType,
		ExecutorName: common.ExecutorName,
		Identifier:   fmt.Sprintf("Application started on %s", time.Now().Format("01-02-2006 15:04:05")),
	}

	err := handler.notifiersHandler.NotifyWithRetry(statusHandlerName, msg)
	if err != nil {
		handler.ErrorEncountered(err)
	}
}

// ErrorEncountered will log the error and record the number of errors encountered
func (handler *statusHandler) ErrorEncountered(err error) {
	if err == nil {
		return
	}

	log.LogIfError(err)

	atomic.AddUint32(&handler.numErrors, 1)
}

// CollectKeysProblems will record the problem on the identifier so the periodic report will include the number of identity failures
func (handler *statusHandler) CollectKeysProblems(messages []common.OutputMessage) {
	handler.mut.Lock()
	for _, msg := range messages {
		handler.problematicKeys[msg.Identifier] = struct{}{}
	}
	handler.mut.Unlock()
}

// Execute will push the notification messages and will clear the internal state of the status handler
func (handler *statusHandler) Execute(_ context.Context) error {
	handler.mut.Lock()
	numErrors := atomic.SwapUint32(&handler.numErrors, 0)

	numProblematicKeys := len(handler.problematicKeys)
	handler.problematicKeys = make(map[string]struct{})
	handler.mut.Unlock()

	messages := []common.OutputMessage{
		handler.createErrorsMessage(numErrors),
		handler.createProblematicKeysMessage(numProblematicKeys),
	}

	return handler.notifiersHandler.NotifyWithRetry(statusHandlerName, messages...)
}

func (handler *statusHandler) createErrorsMessage(numErrors uint32) common.OutputMessage {
	msg := common.OutputMessage{
		Type:         common.InfoMessageOutputType,
		ExecutorName: common.ExecutorName,
		Identifier:   "No application errors occurred",
	}

	if numErrors == 0 {
		return msg
	}

	msg.Type = common.WarningMessageOutputType
	msg.Identifier = fmt.Sprintf("%d application error(s) occurred, please check the app logs", numErrors)

	return msg
}

func (handler *statusHandler) createProblematicKeysMessage(numProblematicKeys int) common.OutputMessage {
	msg := common.OutputMessage{
		Type:         common.InfoMessageOutputType,
		ExecutorName: common.ExecutorName,
		Identifier:   "All monitored metrics are performing as expected",
	}

	if numProblematicKeys == 0 {
		return msg
	}

	msg.Type = common.WarningMessageOutputType
	msg.Identifier = fmt.Sprintf("%d monitored metrics encountered problems", numProblematicKeys)

	return msg
}

func (handler *statusHandler) createCloseMessage() common.OutputMessage {
	msg := common.OutputMessage{
		Type:         common.WarningMessageOutputType,
		ExecutorName: common.ExecutorName,
		Identifier:   "Application closing",
	}

	return msg
}

// SendCloseMessage will send to all notifiers a close message
func (handler *statusHandler) SendCloseMessage() {
	closeMessage := handler.createCloseMessage()

	// nothing to do with the error here, the app is closing anyway, the error will be logged
	_ = handler.notifiersHandler.NotifyWithRetry(statusHandlerName, closeMessage)
}

// IsInterfaceNil returns true if there is no value under the interface
func (handler *statusHandler) IsInterfaceNil() bool {
	return handler == nil
}
